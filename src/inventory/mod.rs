...
impl Inventory {
    ...
    pub fn find_item(&self, item: ItemId) -> Option<usize> {
        self.slots.iter().position(|s| {
            s.as_ref().is_some_and(|stack| stack.item == item)
        })
    }
...
/// Hotbar selection — indexes into the first N slots of the Inventory
#[derive(Component, Clone, Debug)]
pub struct Hotbar {
...
fn hotbar_hud(
    ...
    query: Query<(&Inventory, &Hotbar), With<Player>>,
) {
...
mod tests {
    ...
    fn test_inventory_swap_slots() {
        ...
        assert_eq!(inv.get_slot(0).unwrap().item, ItemId::Block(BlockType::Dirt));
        assert_eq!(inv.get_slot(1).unwrap().item, ItemId::Block(BlockType::Stone));
    }
