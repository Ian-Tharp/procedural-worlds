...
/// Properties for food items — how much health and hunger they restore.
    ...
    for event in death_events.read() {
        if let Ok((mut inventory, transform)) = query.get_mut(event.entity) {
            ...
            if !dropped_items.is_empty() {
                info!(
                    "Player died — dropping {} item stacks as loot",
                    dropped_items.len()
                );
                ...
                .show(ui, |ui| {
                    ...
                    if prompt.available_food.is_some() {
                        ...
                        let restore_text = props
                            .map(|p| format!("+{:.0}❤", p.health_restore))
                            .unwrap_or_default();
                        ui.label(
