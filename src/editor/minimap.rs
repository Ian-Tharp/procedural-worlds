...
fn minimap_render_system(
    ...
    if texture.texture_handle.is_none() {
        let color_image = egui::ColorImage::from_rgba_unmultiplied([size, size], &texture.pixels);
        texture.texture_handle =
            Some(ctx.load_texture("minimap", color_image, egui::TextureOptions::NEAREST));
    }
    ...
    let pos = egui::pos2(screen.max.x - config.size - config.margin, config.margin);
        ...
        .show(ctx, |ui| {
            ...
            frame.show(ui, |ui| {
                ...
                if config.show_chunk_grid {
                    ...
                    let offset_x =
                        (texture.center_x as f32 % CHUNK_SIZE as f32) / config.blocks_per_pixel;
                    let offset_z =
                        (texture.center_z as f32 % CHUNK_SIZE as f32) / config.blocks_per_pixel;
